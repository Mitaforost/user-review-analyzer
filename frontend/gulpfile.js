...
gulp.task('styles', () =>
    gulp.src(paths.styles)
        .pipe(plumber())
        .pipe(sass({ outputStyle: 'expanded' }).on('error', sass.logError)) // Обрабатываем импортированные файлы
        .pipe(autoprefixer())
        .pipe(concat('style.css')) // Убираем .min
...
gulp.task('scripts', () =>
    gulp.src(paths.scripts)
        ...
        .pipe(gulp.dest(`${paths.dist}/js`)) // Копируем файлы без объединения
        .pipe(bs.stream())
);
